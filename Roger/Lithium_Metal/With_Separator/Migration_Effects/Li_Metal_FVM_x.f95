...
module user_input
SAVE    
     parameter(N=2,NJ=13,Numbertimesteps=3.6d3*2000,tmax=3.6d3*2000)
     ! 3.6d3 = 1 hour
     ...
     double precision :: molar_mass_Fe3O4 = 231.533, density_Fe3O4 = 5.15 ! [g/mol],  [g/cm3]
          ...
          ! *********************** INITIAL CONDITIONS ***********************
     double precision :: Phi_1_init=3.3, Phi_2_init = 0.0, c0_init=0.001, cs_init = 1.0d-5   ! initial conditions: voltage, electrolyte conc, solid-state conc.
     ...
     integer(kind=8)  :: SEP_NODE = 7
     double precision :: len_sep  = 1.0, diff_sep
          !
...
contains 
 FUNCTION OSP(c00)
  ...
  double precision :: U_ref = 0.0 ! 3.3
...
!           write the headers on the first entrance into write all voltage
        ...
        h = xx(2) - xx(1)
        N_an_0 = -z_an*u_an*Fconst*cprev(1,1) * (cprev(2,2) - cprev(2,1))/h - & 
        &        diff_an*(cprev(1,2) - cprev(1,1))/h

              c0E        = (cprev(1,j+1) + cprev(1,j))/2.0
              dc0dxE     = cprev(1,j+1) - cprev(1,j)/h
              dPhi_2dxE = cprev(2,j+1) - cprev(2,j)/h

              N_cat_E = -z_cat*u_cat*Fconst*c0E*dPhi_2dxE - diff_cat*dc0dxE
              N_an_E  = -z_an*u_an*Fconst*c0E*dPhi_2dxE   - diff_an*dc0dxE

        u_an = diff_an/(Rigc*Temp)
        u_cat = diff_cat/(Rigc*Temp)

        dPhi_dx_0 = (cprev(2,2) - cprev(2,1))/(xx(2) - xx(1))
        dcdx_0    = (cprev(1,2) - cprev(1,1))/(xx(2) - xx(1))

        dPhi_dx_NJ = (cprev(2,NJ) - cprev(2,NJ-1))/(xx(NJ) - xx(NJ-1))
        dcdx_NJ    = (cprev(1,NJ) - cprev(1,NJ-1))/(xx(NJ) - xx(NJ-1))

        write(*,15) state, t_write, U0, c0*1000, cprev(2,1), &
        & -z_an*u_an*Fconst*cprev(1,1)*dPhi_dx_0, -diff_an*dcdx_0, &
        & -z_an*u_an*Fconst*cprev(1,NJ)*dPhi_dx_NJ, -diff_an*dcdx_NJ, &
        & -z_cat*u_cat*Fconst*cprev(1,1)*dPhi_dx_0, -diff_cat*dcdx_0, &
        & -z_cat*u_cat*Fconst*cprev(1,NJ)*dPhi_dx_NJ, -diff_cat*dcdx_NJ, &
        & (-z_cat*u_cat*Fconst*cprev(1,1)*dPhi_dx_0 - diff_cat*dcdx_0) * Fconst, &
        & U0/cprev(2,1)
...
!             i_rxn_0 = echem_rxn(c0,cs,Phi_1,Phi_2)
            ...
            if (j.NE.1) then
              h = xx(j) - xx(j-1)
              c0W        = (cprev(1,j) + cprev(1,j-1))/2.0
              dc0dxW     = cprev(1,j) - cprev(1,j-1)/h
              dPhi_2dxW = cprev(2,j) - cprev(2,j-1)/h

              N_cat_W = -z_cat*u_cat*Fconst*c0W*dPhi_2dxW - diff_cat*dc0dxW
              N_an_W  = -z_an*u_an*Fconst*c0W*dPhi_2dxW   - diff_an*dc0dxW
            end if
            if (j.NE.NJ) then
              h = xx(j+1) - xx(j)
              c0E        = (cprev(1,j+1) + cprev(1,j))/2.0
          ...
          write(56,15) state, t_write, xx(j), mAhg, LixFe3O4, c0*1000, Phi_2, N_cat_W, N_an_W
...
! Format for terminal
! numbers
 15    format(1(A5,1X), 4(F12.5,1X), 20(ES15.5,1X))
! headers
...
xmax  = 1.0d-0 ! cm
state = 'D'
! negative is plating, positive is stripping

i_applied = -6.36801e-5/2.0
...
! Constant flux condition at the anode

        dE(1,1) = -diff_cat
        fE(1,1) = -z_cat*u_cat*Fconst*dcdxE(2)

        dE(1,2) = -z_cat*u_cat*Fconst*cE(1)
        fE(1,2) = 0.0

        smG(1)  = -i_applied/Fconst + (dE(1,1)*dcdxE(1) + fE(1,1)*cE(1))

! At the anode there is no flux of the anion

        dE(2,1) = -diff_an
        fE(2,1) = -z_an*u_an*Fconst*dcdxE(2)

        dE(2,2) = -z_an*u_an*Fconst*cE(1)
        fE(2,2) = 0.0

        smG(2)  = 0.0 + (dE(2,1)*dcdxE(1) + fE(2,1)*cE(1))
...
! *************** Electrolyte Concentration ****************
!  Fixed Concentration Condition (at the separator)
!     C = cbulk at x = xmax

        rj(1,1) = 1.0
        smG(1)  = cbulk - rj(1,1)*cprev(1,j)

! At the separator the solution potential is arbitrarily set to 0
        rj(2,2) = 1.0
        smG(2)  = 0.0 - rj(2,2)*cprev(2,j)        
...
!           write(*,15) time, cprev(1,1), cprev(1,2), cprev(1,3), &
!           & cprev(1,j-1), cW(1), cprev(1,j), cE(1), cprev(1,j+1)
!           15    format(1(F6.1,1X), 20(ES17.10,1X))

          ! Solution Potential

          dW(2,1) = Fconst*(z_cat*diff_cat + z_an*diff_an)
          dE(2,1) = Fconst*(z_cat*diff_cat + z_an*diff_an)
          fW(2,1) = Fconst**2*(z_cat**2*u_cat + z_an**2*u_an)*dcdxW(2)
          fE(2,1) = Fconst**2*(z_cat**2*u_cat + z_an**2*u_an)*dcdxE(2)
          
          dW(2,2) = Fconst**2*(z_cat**2*u_cat + z_an**2*u_an)*cW(1)
          dE(2,2) = Fconst**2*(z_cat**2*u_cat + z_an**2*u_an)*cE(1)
          fW(2,2) = 0.0
          fE(2,2) = 0.0

          smG(2)  = 0.0 &
          &         - (fW(2,1)*cW(1) + dW(2,1)*dcdxW(1)) &
          &         + (fE(2,1)*cE(1) + dE(2,1)*dcdxE(1))
